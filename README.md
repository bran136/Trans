...
- 页面右下角显示“正式版本 + 构建指纹”，例如 `v1.0+a07a6f8b`；页面资源变化后指纹和资源 URL 会自动更新。
...
可以直接确认新页面已经生效。只有准备新的正式 Release 时才更新 `VERSION`。修改后端
...
“服务器固定”不是第四份音频，而是把对应服务器缓存标记为不参与自动淘汰。

浏览器内存最多保留 12 句、约 64MB；刷新页面或切换章节、音色和听书配置时会释放，用于加速刚播放或预取的句子。

服务器磁盘缓存：

- 目录：`./path_dir/reader_data/tts_cache`
- 示例配置上限：`TTS_CACHE_LIMIT_MB=8192`
- 示例配置有效期：`TTS_CACHE_TTL_DAYS=90`
- 普通缓存按容量和有效期自动淘汰；固定缓存不自动淘汰，取消固定后恢复普通缓存策略。
- 缓存键包含文本、模型、音色、音频格式和风格提示；相同配置命中时不会调用 MiMo API。

听书会按句子长度和播放倍速预取后续内容，并复用进行中的相同请求。播放优先级为浏览器内存、IndexedDB、服务器缓存、MiMo 实时生成。

为降低手机浏览器连续播放短音频时意外暂停的概率，浏览器可能临时合并相邻音频；服务端缓存、句子高亮、阅读进度和定时暂停仍按原句处理。

### 离线听书缓存

在“书籍管理”中，每本书都可以按章节执行：

- 章节状态按“服务器固定数/总句数、本机缓存数/总句数”显示，便于直接确认缓存完整度。
- 服务器固定：复用当前朗读配置的已有音频，缺失句子调用 MiMo 生成后固定。
- 固定并下载到本地：完成服务器固定后，将音频写入当前浏览器的 IndexedDB。
- 删除本地缓存：只删除当前浏览器、当前朗读配置下选定章节的副本。
- 取消服务器固定：只移除固定标记，不删除各浏览器已经下载的副本。
- 服务器最多同时处理 2 个章节，每章内部严格逐句串行。
- 任务进行时可以取消；服务器会收尾正在处理的句子，本机下载会立即停止，已完成部分保留。

本机缓存按书籍、朗读配置、章节和句子隔离。删除书籍或修改章节结构时，服务器固定引用会清除，执行操作的浏览器也会尝试清理该书的本机副本；其他设备中的副本需要在对应设备处理。
...
导入任务和离线听书任务的进度、取消状态保存在进程内存中，因此应使用单个 Gunicorn worker；可以通过线程处理并发网页请求。

```bash
gunicorn --workers 1 --threads 4 --bind 127.0.0.1:31000 app:app
```
...
推送到 `master` 或向 `master` 提交 Pull Request 时，GitHub Actions 会在 Python 3.11、3.12 和 3.13 上安装依赖、检查语法并导入应用。工作流见 [`.github/workflows/ci.yml`](.github/workflows/ci.yml)。
